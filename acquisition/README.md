...
├── config.demo.yaml       # 合成数据验证配置(back=1)
├── scripts/
│   ├── start_zenohd.sh            # 手动启动 Zenoh Router
│   ├── install_zenohd_service.sh   # 安装常驻 systemd 服务
│   ├── demo_manus_pub.py           # 合成 Manus 骨架发布器(30Hz)
...
└── tests/                 # 47 项:单测 + zenoh 集成 + HDF5 往返 + pty 键盘
```
...
```bash
# 1. Router(两种方式任选)
pixi run start-router                    # 前台运行
bash scripts/install_zenohd_service.sh --start   # 或装常驻 systemd 服务

# 2. 合成数据(无真实设备)
pixi run demo-mocap                      # mocap 合成帧(60Hz,含背部刚体)
pixi run demo-manus                      # manus 合成骨架(30Hz)

# 3. 采集(带可视化,浏览器 http://127.0.0.1:8081)
pixi run record
# 按键: r 开始录制 / 空格 暂停恢复 / s 保存 / d 丢弃 / q 退出

# 4. 检查与回放
pixi run inspect -- captures/take_001_*.h5
pixi run replay -- captures/take_001_*.h5 --json out.jsonl
```

> 合成 mocap 帧的背部刚体 ID 是 1,验证时用 `--config` 指定临时配置把
> `rigid_bodies.back` 改为 1(见 `scripts/` 旁 `config.demo.yaml`)。
>
> `pixi run demo-mocap` 复用上级 mocap 项目(合并后的主文件夹)的合成发布器
> (60Hz,仅 1 个刚体 id=1,无物体刚体);真实环境下 Windows publisher 直接提供
> 全部刚体(背部+物体)。端到端一键验证:`pixi run python scripts/e2e_check.py --seconds 5`。
>
...
时间基准:各流独立 `t_ubuntu_ns`(接收端墙钟)。手部 30Hz 与动捕 120Hz 帧率不同,
...
- **端到端**:真实动捕流(120.2Hz,刚体 1/2/3)+ 合成手部(29.7Hz)→ 按键录制 →
